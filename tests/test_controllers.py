...
class TestBookController():
    ...
    def setup_book(self, book_controller):
        book_controller.add_book(
            'Some Title', 'Some Author', '1900', 'Some Genre', '123456')
        book_controller.add_book(
            'Another Title', 'Another Author', '2000', 'Another Genre', '789465')
        return book_controller

    @pytest.fixture
    def setup_books_update(self, book_controller):
        book_controller.add_book(
            'Original title', 'Original author', '1900', 'Original genre', '11111')
        book_controller.add_book(
            'Original title', 'Original author', '1900', 'Original genre', '22222')
        book_controller.add_book(
            'Original title', 'Original author', '1900', 'Original genre', '33333')
        book_controller.add_book(
            'Original title', 'Original author', '1900', 'Original genre', '44444')
        return book_controller
    ...
    @pytest.mark.parametrize(
        "title, author, year, genre, code",
        [
            ('Some Title', 'Some Author', 'ABCD',
             'Some Genre', '12345'),  # year isn't numeric
            ('Some Title', 'Some Author', '1900',
             'Some Genre', 'ABCD'),  # code isn't numeric
            # year and code aren't numeric
            ('Some Title', 'Some Author', 'ABCD', 'Some Genre', 'BCDA'),
        ]
    )
...
class TestUserController():
    ...
    def test_delete_user_returns_none_if_user_code_doesnt_exists(self, setup_user):
        delete_code = '123456'
        assert setup_user.find_by_user_code(delete_code) is None